...
var (
	...
	bytesPool = NewLimitedBytes(256, walEncodeBufSize*2)
)
...
func New(settings *Settings, l *zap.Logger) (*WAL, error) {
	...
	wal := &WAL{
		...
		stats:       &WALStatistics{},
		limiter:     rate.NewLimiter(rate.Limit(defaultWaitingWALWrites), 0),
		logger:      logger,
		traceLogger: logger,
...
func (l *WAL) Open() error {
	...
	l.traceLogger.Info(fmt.Sprintf("mycenae WAL starting with %d segment size", l.SegmentSize))
	l.traceLogger.Info(fmt.Sprintf("mycenae WAL writing to %s", l.path))
...
func (l *WAL) writeToLog(entry WALEntry) (int64, error) {
