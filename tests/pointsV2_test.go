...
type PayloadPoints struct {
	ID map[string]Points `json:"payload"`
}
...
type PayloadPointsNull struct {
	ID map[string]PointsNull `json:"payload"`
}

type PointsNull struct {
	Data DataNull `json:"points"`
}
...
func postPointsAndCheck(t *testing.T, payload, id string, code, count, total, ts int) PayloadPoints {
	...
	assert.Equal(t, count, payloadPoints.ID[id].Data.Count)
	assert.Equal(t, total, payloadPoints.ID[id].Data.Total)
	assert.Equal(t, ts, len(payloadPoints.ID[id].Data.Ts))

	return payloadPoints
}

func TestPointsV2PointsLimitTrueMenorQueLimite(t *testing.T) {
	payload := `{
		"downsample": {
			"pointLimit": true,
			"totalPoints": 150
	...
	for _, value := range payloadPoints.ID[hashMapPV2["ts1"]].Data.Ts {

		assert.Exactly(t, i, value[1], "they should be equal")
		i++

		assert.Exactly(t, dateStart, value[0], "they should be equal")
...
*/
func TestPointsV2LimitTrueMaiorLimiteMenor2(t *testing.T) {
	payload := `{
		"downsample": {
			"pointLimit": true,
			"totalPoints": 90
...
func TestPointsV2LimitTrueMaiorLimiteMaior2(t *testing.T) {
	payload := `{
		"downsample": {
			"pointLimit": true,
			"totalPoints": 50
...
func TestPointsV2LimitTrueNoPoints(t *testing.T) {
	...
	assert.Equal(t, 204, code, "they should be equal")
...
func TestPointsV2LimitTrueAproxMedia(t *testing.T) {
	...
	payloadPoints := PayloadPointsNull{}
	...
	assert.Equal(t, 34, payloadPoints.ID[hashMapPV2["ts2_3"]].Data.Count, "they should be equal")
	assert.Equal(t, 90, payloadPoints.ID[hashMapPV2["ts2_3"]].Data.Total, "they should be equal")
	assert.Equal(t, 34, len(payloadPoints.ID[hashMapPV2["ts2_3"]].Data.Ts), "they should be equal")
	i := 0
	dateStart := 1448452440000.0
	for _, value := range payloadPoints.ID[hashMapPV2["ts2_3"]].Data.Ts {

		if value[0].(float64) < 1448452800000.0 {
			assert.Exactly(t, nil, value[1], "they should be equal", payload)
			...
			assert.Exactly(t, media, value[1].(float64), "they should be equal", payload)
			i++
		}
		assert.Exactly(t, dateStart, value[0].(float64), "they should be equal", payload)
		dateStart += 180000
...
func TestPointsV2LimitTrueAproxMediaExactBeginAndEnd(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[hashMapPV2["ts2_3"]].Data.Ts {
		...
		media := float64((i + i + 1 + i + 2) / 3)
		assert.Exactly(t, media, value[1], "they should be equal")
		i++
...
func TestPointsV2LimitTrueAproxMin(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[hashMapPV2["ts2_3"]].Data.Ts {
		...
		if value[0].(float64) < 1448452800000.0 {
			assert.Exactly(t, nil, value[1], "they should be equal")

		} else if value[0].(float64) > 1448458020000.0 {
			assert.Exactly(t, nil, value[1], "they should be equal")

		} else {
			assert.Exactly(t, i, value[1].(float64), "they should be equal")
			i++
		}
		assert.Exactly(t, dateStart, value[0].(float64), "they should be equal")
		dateStart += 180000
...
func TestPointsV2LimitTrueAproxSumExactBeginAndEnd(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[hashMapPV2["ts2_3"]].Data.Ts {
		...
		assert.Exactly(t, sum, value[1], "they should be equal")
		i++
...
func TestPointsV2LimitTrueAproxPnt(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[hashMapPV2["ts2_3"]].Data.Ts {
		...
		assert.Exactly(t, 3.0, value[1], "they should be equal")
...
func TestPointsV2LimitTrueAproxMaxHour(t *testing.T) {
	...
	assert.Equal(t, 200, code, "they should be equal")
	assert.Equal(t, 120, payloadPoints.ID[hashMapPV2["ts3"]].Data.Count, "they should be equal")
	assert.Equal(t, 480, payloadPoints.ID[hashMapPV2["ts3"]].Data.Total, "they should be equal")
	assert.Equal(t, 120, len(payloadPoints.ID[hashMapPV2["ts3"]].Data.Ts), "they should be equal")
	i := 15.0
	dateStart := 1448452800000.0
	for _, value := range payloadPoints.ID[hashMapPV2["ts3"]].Data.Ts {
...
func TestPointsV2LimitTrueAproxMaxDay(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[hashMapPV2["ts3"]].Data.Ts {
		...
		if int64(value[0]) < 1449100800000 {
...
func TestPointsV2LimitTrueAproxMaxWeek(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[hashMapPV2["ts4"]].Data.Ts {
		...
		assert.Exactly(t, float64(date), value[0], "they should be equal")
...
func TestPointsV2LimitTrueAproxMaxMonth(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[hashMapPV2["ts4"]].Data.Ts {
		...
		if int64(value[0]) <= 1501545600000 {

			assert.Exactly(t, i, value[1], "they should be equal")
			i += 6.5

		} else if int64(value[0]) == 1509494400000 {

			i = 57.0
			assert.Exactly(t, i, value[1], "they should be equal")

		} else if int64(value[0]) == 1517443200000 {

			i = 63.0
			assert.Exactly(t, i, value[1], "they should be equal")

		} else if int64(value[0]) == 1525132800000 {

			i += 6.5
			assert.Exactly(t, i, value[1], "they should be equal")

		} else if int64(value[0]) == 1533081600000 {

			i += 7.0
			assert.Exactly(t, i, value[1], "they should be equal")

		} else if int64(value[0]) <= 1541030400000 {

			i += 6.5
			assert.Exactly(t, i, value[1], "they should be equal")

		} else if int64(value[0]) == 1548979200000 {

			i += 6.0
			assert.Exactly(t, i, value[1], "they should be equal")

		} else if int64(value[0]) == 1556668800000 {
...
func TestPointsV2LimitTrueAvgMaiorLimit2(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[hashMapPV2["ts2_3"]].Data.Ts {
		...
		assert.Exactly(t, avg, value[1], "they should be equal")
		i++
...
func TestPointsV2MergeDateLimit(t *testing.T) {
	...
	assert.Equal(t, 200, code, "they should be equal")
	assert.Equal(t, 100, payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Count, "they should be equal")
	assert.Equal(t, 100, payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Count, "they should be equal")
	assert.Equal(t, 100, len(payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Ts), "they should be equal")

	i := 0.0
	dateStart := 1448452800000.0
	for _, value := range payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Ts {

		assert.Exactly(t, i+i, value[1], "they should be equal")
		i++
...
func TestPointsV2LimitTrueMergeBiggerPointsSameDate(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Ts {
		...
		assert.Exactly(t, i*2, value[1], "they should be equal")
		i++
	}
}
...
func TestPointsV2MergeMax(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Ts {
		...
		assert.Exactly(t, j, value[1], "they should be equal")
		j += 5
	}
}
...
func TestPointsV2MergeSumEnabledPnt(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Ts {
		...
		if value[0] < 1448458020000 {

			assert.Exactly(t, i, value[1], "they should be equal")

		} else {

			assert.Exactly(t, 4.0, value[1], "they should be equal")
...
func TestPointsV2MergeTimeDiffDownsampleExactBeginAndEnd(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Ts {
		...
		if value[0] < 1448458740000 {

			assert.Exactly(t, i, value[1], "they should be equal")

		} else {

			assert.Exactly(t, 2.0, value[1], "they should be equal")
...
func TestPointsV2MergeTimeDiffDownsample(t *testing.T) {
	...
	for _, value := range payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Ts {
		...
		if value[0] == 1448452800000 {
			assert.Exactly(t, 1.0, value[1], "they should be equal")
			i += 3

		} else if value[0] == 1448458740000 {
			assert.Exactly(t, 99.0, value[1], "they should be equal")
			i += 3
...
func TestPointsV2NullValuesEnabled3Min(t *testing.T) {
	...
	i := 0.0
	for _, value := range payloadPoints.ID[hashMapPV2["ts9_2"]].Data.Ts {
...
func TestPointsV2BothValuesNullMergeAndDownsample(t *testing.T) {
	...
	assert.Equal(t, 200, code, "they should be equal")
	assert.Equal(t, 30, payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Count, "they should be equal")
	assert.Equal(t, 60, payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Total, "they should be equal")
	assert.Equal(t, 30, len(payloadPoints.ID[ksMycenae+"|merged:["+nameTS+"]"].Data.Ts), "they should be equal")
...
func TestPointsV22Merge(t *testing.T) {
	...
	i := 0.0
	j := 0.0
	for _, value := range payloadPoints.ID[ksMycenae+"|merged:["+nameTS1+"]"].Data.Ts {
	...
	assert.Equal(t, 30, payloadPoints.ID[ksMycenae+"|merged:["+nameTS2+"]"].Data.Count, "they should be equal")
	assert.Equal(t, 120, payloadPoints.ID[ksMycenae+"|merged:["+nameTS2+"]"].Data.Total, "they should be equal")
	assert.Equal(t, 30, len(payloadPoints.ID[ksMycenae+"|merged:["+nameTS2+"]"].Data.Ts), "they should be equal")

	i = 0.0
	j = 0.0
	for _, value := range payloadPoints.ID[ksMycenae+"|merged:["+nameTS2+"]"].Data.Ts {
...
func TestPointsV2MoreThanOneTS(t *testing.T) {
	...
	assert.Equal(t, 100, payloadPoints.ID[hashMapPV2["ts1"]].Data.Count, "they should be equal")
	assert.Equal(t, 100, payloadPoints.ID[hashMapPV2["ts1"]].Data.Total, "they should be equal")
	assert.Equal(t, 100, len(payloadPoints.ID[hashMapPV2["ts1"]].Data.Ts), "they should be equal")

	dateStart := 1448452800000.0
	count := 0.0
	for _, value := range payloadPoints.ID[hashMapPV2["ts1"]].Data.Ts {

		assert.Exactly(t, count, value[1], "they should be equal")
		count++
	...
	assert.Equal(t, 25, payloadPoints.ID[ts10ID].Data.Count, "they should be equal")
	assert.Equal(t, 25, payloadPoints.ID[ts10ID].Data.Total, "they should be equal")
	assert.Equal(t, 25, len(payloadPoints.ID[ts10ID].Data.Ts), "they should be equal")

	dateStart = 1448452800000.0
	count = 0.0
	for _, value := range payloadPoints.ID[ts10ID].Data.Ts {
